...
(defun direx:do-chmod-file ()
  ...
  (let* ((item (direx:item-at-point!))
         ...
         (orig-modes-str (nth 8 (file-attributes filename 'string)))
         (modes (read-string (format "Change mode of %s[%s]: "
                                     filename orig-modes-str)))
...
(defun direx:do-chxxx (program attr orig filename)
  (let* ((prompt (format "Change %s of %s[%s]: "
                         attr (file-name-nondirectory filename) orig))
...
(defun direx:do-chgrp ()
  ...
  (let* ((item (direx:item-at-point!))
         (file (direx:item-tree item))
         (filename (direx:file-full-name file))
         (orig (nth 3 (file-attributes filename 'string))))
    (direx:do-chxxx "chgrp" "Group" orig filename)))
...
(defun direx:do-chown ()
  ...
  (let* ((item (direx:item-at-point!))
         (file (direx:item-tree item))
         (filename (direx:file-full-name file))
         (attrs (file-attributes filename 'string))
         (orig (format "%s:%s" (nth 2 attrs) (nth 3 attrs))))
    (direx:do-chxxx "chown" "Owner" orig filename)))
...
(defun direx:do-touch ()
  ...
  (let* ((item (direx:item-at-point!))
         ...
         (mtime (format-time-string
                 "%Y%m%d%H%M.%S" (nth 5 (file-attributes filename))))
         (prompt (format "Change Timestamp of %s[%s]: "
                         (file-name-nondirectory filename) mtime))
